...
public class MathUtil {
    ...
    public static float lerpValues(AnimationPoint animationPoint, EasingType easingType, Function<Double, Double> customEasingMethod, List<Double> easingArgs) {
		...
		List<Double> args = animationPoint.keyframe == null ? null : animationPoint.keyframe.easingArgs;
		if(easingArgs != null) {
			args = easingArgs;
		}

        double ease = EasingManager.ease(animationPoint.currentTick / animationPoint.animationEndTick, easingType, args);
